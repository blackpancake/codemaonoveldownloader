# head
import os
# os.chdir(os.path.split(__file__)[0])       #如果是用usb-python运行，请把这行注释解开
...
class novel:
    def __init__(self, number, ip, port):
        ...
        try:
            self.info = requests.get('https://api.codemao.cn/api/fanfic/' +
                                     self.number,
                                     headers=rdags(),
                                     proxies=self.rdip(),
                                     timeout=1.5)
        ...
        self.isghost = self.fm_url.split('/')[:3] != [
            'https:', '', 'static.codemao.cn'
        ]
        try:
            self.picdata = requests.get(self.fm_url,
                                        headers=rdags(),
                                        proxies=self.rdip(),
                                        timeout=1.5).content
        except Exception as F:
...
class App:
    ...
    def mainloop(self):
        testingthread = threading.Thread(target=self.testNetwork, daemon=True)
        testingthread.start()
        self.body.mainloop()
    ...
    def testNetwork(self):
        while True:
            if not isConnected():
                mBox.showerror('错误', '你还未联网呢！赶快联网吧QWQ\n要不然无法使用本软件')
            time.sleep(3)
    ...
    def when_close(self):
        if self.isdowning:
            ...
            answer = mBox.askyesno("警告", "你确定要关闭窗口吗？")
            if answer:
                self.body.quit()
                self.body.destroy()
